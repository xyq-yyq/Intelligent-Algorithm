"""PMITS 选择 vs 四种单一哈希方法 对比脚本。

对比对象：
    PMITS       从 288 位池中优化选出 ld 个比特（当前最优选择算法）
    DeepHN      仅使用 DeepHN 方法的前 ld 位（比特 0-71）
    HashNet     仅使用 HashNet 方法的前 ld 位（比特 72-143）
    IDeepHN     仅使用 IDeepHN 方法的前 ld 位（比特 144-215）
    LSH         仅使用 LSH 方法的前 ld 位（比特 216-287）

目的：
    将 PMITS 的选择能力与四种原始哈希方法直接对比。
...
HASH_METHODS = {
    'DeepHN':  (0, 72),       # 比特   0– 71
    'HashNet': (72, 144),     # 比特  72–143
    'IDeepHN': (144, 216),    # 比特 144–215
    'LSH':     (216, 288),    # 比特 216–287
}
...
def eval_method(method_name, ld, pool_test, test_labels, verbose=True):
    ...
    t0 = time.time()
    start, end = HASH_METHODS[method_name]
    idx = np.arange(start, start + ld)       # 取该方法的前 ld 个比特
    codes = pool_test[idx].T.astype(np.int8)
    elapsed = time.time() - t0
    map_val, prec, rec, ph2 = evaluate_codes(codes, test_labels)
    if verbose:
        print(f"  {method_name:<8s} LD={ld:>3}: MAP={map_val*100:.2f}%, "
...
def plot_comparison(results, ld_list):
    # 生成 PMITS vs 四种哈希方法的对比折线图。

    method_names = ['PMITS'] + list(HASH_METHODS.keys())
    # 颜色：PMITS 用醒目红色，四种方法用 Set2 调色板
    colors = ['#E63946', '#3B7DD8', '#2ECC40', '#E67E22', '#8E44AD']
    markers = ['D', 'o', 's', '^', 'v']
    ...
    plt.tight_layout()
    save_path = os.path.join(POOL_DIR, 'comparison_methods.png')
...
def plot_bar_comparison(results, ld_list):
    """生成各 LD 下 PMITS vs 四种哈希方法的 MAP 柱状对比图。

    每个 LD 对应一个小柱状图，按 3×3 网格排列。
    """
    method_names = ['PMITS'] + list(HASH_METHODS.keys())
    colors = ['#E63946', '#3B7DD8', '#2ECC40', '#E67E22', '#8E44AD']
    n = len(ld_list)
    ...
    plt.tight_layout()
    save_path = os.path.join(POOL_DIR, 'comparison_methods_bars.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
...
def main():
    ...
    print("=" * 60)
    print("PMITS 选择 vs 四种单一哈希方法 对比")
    print("=" * 60)
    print(f"基准种子: {SEED}, 实验次数: {NUM_RUNS} 次取均值")
    print(f"对比方法: PMITS + {list(HASH_METHODS.keys())}")
    print(f"哈希码长度: {LD_LIST}")
    print(f"池结构: DeepHN(0-71) HashNet(72-143) IDeepHN(144-215) LSH(216-287)")
    ...
    for ld in LD_LIST:
        ...
        print(f"  LD={ld}: DeepHN/ HashNet/ IDeepHN/ LSH 完成")
    ...
    print(header)
    print('─' * len(header))
    for name in ['PMITS'] + list(HASH_METHODS.keys()):
        row = f"{name:<10}"
        for ld in LD_LIST:
    ...
    # 保存
    save_path = os.path.join(POOL_DIR, 'comparison_methods.npz')
